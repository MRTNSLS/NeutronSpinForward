...
@timeit_decorator
def calc_precession(nNeutrons, nAngles, angles, wavelengths, voxel_index, voxel_data, B):
            ...
            for n in range(0,nNeutrons):
                ...
                for ind_l,lam in enumerate(wavelengths):
                    ...
                    for ind,v in enumerate(list(vs.flat)):
                        ...
                        q = q*calc_quaternion(Bs,lam,Ls[ind])
...
def process_single_combination(a, p, n, Br, wavelengths, voxel_index, voxel_data, p_start):
    # Starting quaternion
    qs = quat.quaternion(*np.array([1., 0., 0., 0.]))

    measurement = np.empty((len(wavelengths), 3, 3), dtype=float)
    ...
    for ind_l, lam in enumerate(wavelengths):
        ...
        p_end = rotate_vec(p, q=q)
        measurement[ind_l] = p_end

    return n, a, p_start.index(p), measurement

@timeit_decorator
def calc_precession_parallel(nNeutrons, nAngles, angles, wavelengths, voxel_index, voxel_data, B):
    p_start = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
    
    measurement = np.empty((nNeutrons, nAngles, len(wavelengths), 3, 3), dtype=float)

    # Precompute the rotated B fields for each angle
    Br_list = []
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        for a in range(0, nAngles):
            Br = Br_list[a]  # Use precomputed Br for this angle
            for ind_p, p in enumerate(p_start):
                for n in range(0, nNeutrons):
                    futures.append(executor.submit(process_single_combination, a, p, n, Br, wavelengths, voxel_index, voxel_data, p_start))

        for future in concurrent.futures.as_completed(futures):
            n, a, p_ind, result = future.result()
            measurement[n][a][:] = result
...
if __name__ == "__main__":
    ...
    measurement = calc_precession_parallel(nNeutrons, nAngles, angles, wavelengths, voxel_index, voxel_data, B)
    measurement2 = calc_precession(nNeutrons, nAngles, angles, wavelengths, voxel_index, voxel_data, B)
    ...
    for sp in range(0,3):
        for sa in range(0,3):
                axs[sa, sp].imshow(measurement[:, :, 0, sp, sa], interpolation='none', vmin=-1, vmax=1)
                axs[sa+4, sp].imshow(measurement2[:, :, 0, sp, sa], interpolation='none', vmin=-1, vmax=1)

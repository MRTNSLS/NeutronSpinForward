...
# DISABLE TO RETURN TO GOOD 3x3 results
...
model.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mae']) # DISABLE TO RETURN TO GOOD 3x3 results
...
# Callback for saving model metadata
class MetadataCallback(Callback):
    def __init__(self, batch_size, epochs, filepath):
        super().__init__()
        self.batch_size = batch_size
        self.epochs = epochs
        self.filepath = filepath

    # This is automatically called when the training begins
    def on_train_begin(self, logs=None):
        # Create a dictionary with metadata (now self.model is available)
        metadata = {
            'batch_size': self.batch_size,
            'epochs': self.epochs,
            'optimizer': str(self.model.optimizer.get_config()),  # Access the optimizer
            'loss': self.model.loss,  # Loss function
            'metrics': self.model.metrics_names  # Metrics
        }

        # Save metadata to a JSON file
        with open(self.filepath, 'w') as f:
            json.dump(metadata, f, indent=4)
        print(f"Metadata saved to {self.filepath}")
...
# Define the callback without passing the model directly
metadata_callback = MetadataCallback(
    batch_size=batch_size,
    epochs=epochs,
    filepath=f'models/my_model_meta_{ver}.json'
)
...
history = model.fit(
    ...
    callbacks=[plot_and_stop_callback, metadata_callback]
)
